...
export default function Category() {
  ...
  async function onFetchMoreLists(){
    try {
      const listingRef = collection(db, "listings")
      const q = query(
        listingRef, 
        ...
        <>
          ...
          {lastFetch && (
            <div className="flex justify-center items-center">
              <button onClick={onFetchMoreLists} className="bg-white px-3 py-1.5 text-gray-700 border border-gray-300 mb-6 mt-6 hover:border-slate-600 rounded transition duration-200 ease-in-out">Load more</button>
            </div>
          )}
        </>
      ) : (
