...
export default function CreateListing() {
    ...
    const {type, name, bedrooms, bathrooms, parking, furnished, address, description, offer, regularPrice, discount, latitude, longitude, images} = formData;
    ...
    async function onSubmit(e){
        ...
        if(+discount >= +regularPrice){
            setLoading(false)
            toast.error("The Regular Price must be higher than the Discount Price");
            return;
        }
        if(images.length > 6){
            setLoading(false)
        ...
        // Below is the code for google map geolocation API

        let geolocation = {}
        let location
        if(geolocationEnabled){
            const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${address}&kay=${process.env.REACT_APP_GEOCODE_API_KEY}`);
    ...
    <main className="max-w-md px-2 mx-auto">
        ...
        <form onSubmit={onSubmit}>
            ...
            <input 
                ...
                minLength="10" 
                required 
                className="w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-900 focus:bg-white focus:border-slate-900 mb-6"
            />
                ...
                <div className="flex space-x-6 mb-6">
                    <div className="">
                        ...
                        <input 
                            ...
                            min="-90"
                            max="90"
                            className="w-full px-4 py-2 text-lg text-gray-700 bg-white border border-gray-300 rounded transition duration-300 ease-in-out focus:bg-white focus:text-gray-700 focus:border-slate-600 text-center"
                        />
                    </div>
                    <div className="">
            ...
            <div className="flex items-center mb-6">
                <div className="">
                    ...
                    <div className="flex w-full justify-center items-center space-x-6">
                        ...
                        min="50" 
                        max="400000000" 
                        required className="w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-gray-900 text-center "/>
                        {type === "rent" && (
                        <div className=" w-full">
                            <p className="text-md whitespace" >$ / Month</p>
                        </div>
                    )}
                    </div>
                </div>
            ...
            {offer && (
                  ...
                  <div className="">
                      ...
                      <div className="flex w-full justify-center items-center space-x-6">
                          ...
                          max="400000000" 
                          required={offer}
                          className="w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-gray-900 text-center "/>
                          {type === "rent" && (
                          <div className=" w-full">
                              <p className="text-md whitespace" >$ / Month</p>
                          </div>
            ...
            <button type="submit" className="mb-6 w-full px-7 py-3 bg-blue-600 text-white font-medium text-sm uppercase rounded shadow-md hover:bg-blue-900 hover:shadow-lg focus:bg-blue-400 focus:shadow-2xl active:bg-blue-100 active:shadow-lg transition duration-1000 ease-in-out ">Create Listing</button>
        </form>
    </main>
  )
}
