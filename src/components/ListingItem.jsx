...
export default function ListingItem({listing, id, onEdit, onDelete}) {
  return (
    <li className=" relative bg-white flex flex-col justify-between items-center shadow-md hove:shadow-xl rounded-md overflow-hidden transition-shadow duration-200 mb-6 mr-2">
        <Link to={`/category/${listing.type}/${id}`}>
            <img 
                className="h-[170px] w-full object-cover hover:scale-110 transition-scale duration-200 ease-in-out"
                loading="lazy"
            ...
            <div className="w-full p-[10px]">
                <div className="flex items-center space-x-1">
                    ...
                    <p className="font-semibold text-sm mb-[2px] text-gray-600 line-clamp-1">{listing.address}</p>
                </div>
                <p className="font-semibold m-0 text-xl line-clamp-1">{listing.name}</p>
